/*
 * Graph Plotter by Arefin
 * This program allows users to plot and visualize various mathematical functions
 * including trigonometric, polynomial, logarithmic, and geometric shapes
 * with interactive controls for colors, scaling, and panning.
 */
 ...
 * Define the basic parameters for the application window
...
/*
 * Grid and Axis Constants
 * Define visual properties of the coordinate system
 * including colors, spacing, and dimensions
 */
// Constants for grid
...
/*
 * Input and Scaling Constants
 * Define limits and default values for user input
 * and view scaling operations
 */
...
/*
 * Function Prototypes
 * Declaration of all function interfaces used in the program
 * grouped by functionality
 */
// Parser Functions - Convert string inputs to mathematical parameters
void removeWhitespaces(char *str);                // Removes all whitespace from input string
float parseNumber(char **ptr);                    // Parses a floating point number from string
// Function Equation Readers - Parse different types of function equations
int readPolynomial(const char* equation, float *a4, float *a3, float *a2, float *a1, float *a0);                      // Reads polynomial coefficients
int readTrigFunction(const char* equation, float *A, float *B, float *C, float *D, char *funcType);                    // Reads trigonometric function parameters
...
double scaleX = INITIAL_SCALE_X, scaleY = INITIAL_SCALE_Y;  // Current view scaling
double step = STEP_INITIAL;                                  // Step size for plotting
double offsetX = 0, offsetY = 0;                            // View pan offset

// Function Visibility Flags
bool showSin = false;          // Toggle sine function display
bool showCos = false;          // Toggle cosine function display
bool showTan = false;          // Toggle tangent function display
bool showASin = false;         // Toggle arcsine function display
bool showACos = false;         // Toggle arccosine function display
bool showATan = false;         // Toggle arctangent function display
bool showExp = false;          // Toggle exponential function display
bool showLog = false;          // Toggle logarithmic function display
bool showLn = false;           // Toggle natural logarithm function display
bool showPoly = false;         // Toggle polynomial function display
bool showCircle = false;       // Toggle circle display
bool showEllipse = false;      // Toggle ellipse display
bool showParabola = false;     // Toggle parabola display
bool showHyperbola = false;    // Toggle hyperbola display

// UI State Variables
bool showHelpScreen = false;   // Toggle help screen
bool isSettingColor = false;   // Color selection mode active
bool showGridFlag = false;     // Toggle grid display
bool isEnteringEquation = false; // Equation input mode active
bool isPanning = false;        // Panning mode active
...
/*
 * Data Structures
 * Define the structure of various mathematical functions
 * and their parameters
 */
// Color Structure - RGB color representation
typedef struct {
...
// Mathematical Function Structures
typedef struct {
    float A;    // Amplitude
    float B;    // Frequency
    float C;    // Phase shift
...
/*
 * Color Presets
 * Predefined color schemes for different function types
 */
...
int currentPreset = 1;  // 1 is the index for Neon preset
...
void applyColorPreset(int presetIndex) {
    // Applies selected color preset to all functions
...
void removeWhitespaces(char *str) {
    // Removes whitespace characters from input string
    char *i = str;
    char *j = str;
    while (*j != 0) {
        *i = *j++;
...
// Mathematical Function Parsers
float parseNumber(char **ptr) {
    // Parses floating point number from string pointer
    // Handles sign, integer part, and decimal part
    float num = 0;
    int sign = 1;
...
int readPolynomial(const char* equation, float *a4, float *a3, float *a2, float *a1, float *a0) {
    // Parses polynomial equation of form ax^4 + bx^3 + cx^2 + dx + e
    // Returns coefficients through pointer parameters
...
int readTrigFunction(const char* equation, float *A, float *B, float *C, float *D, char *funcType) {
    // Parses trigonometric function equation of form y = A*sin(B*x + C) + D
    // Returns parameters through pointer variables
...
int readInverseTrigFunction(const char* equation, float *A, float *B, float *C, float *D, char *funcType) {
    // Parses inverse trigonometric function equation of form y = A*asin(B*x + C) + D
    // Returns parameters through pointer variables
    *A = 1.0f; *B = 1.0f; *C = 0.0f; *D = 0.0f;
    char cleanEq[MAX_INPUT_LENGTH];
    strncpy(cleanEq, equation, MAX_INPUT_LENGTH - 1);
...
int readExponentialFunction(const char* equation, float *A, float *B, float *C, float *D) {
    // Parses exponential function equation of form y = A*exp(B*x + C) + D
    // Returns parameters through pointer variables
    *A = 1.0f; *B = 1.0f; *C = 0.0f; *D = 0.0f;
    char cleanEq[MAX_INPUT_LENGTH];
    strncpy(cleanEq, equation, MAX_INPUT_LENGTH - 1);
...
int readLogFunction(const char* equation, float *A, float *B, float *C, float *D, char *funcType) {
    // Parses logarithmic function equation of form y = A*log(B*x + C) + D
    // Returns parameters through pointer variables
    *A = 1.0f; *B = 1.0f; *C = 0.0f; *D = 0.0f;
    char cleanEq[MAX_INPUT_LENGTH];
    strncpy(cleanEq, equation, MAX_INPUT_LENGTH - 1);
...
int readCircle(const char *equation, float *h, float *k, float *r) {
    // Parses circle equation of form (x - h)^2 + (y - k)^2 = r^2
    // Returns parameters through pointer variables
    char cleanEq[MAX_INPUT_LENGTH];
    strcpy(cleanEq, equation);
    removeWhitespaces(cleanEq);
...
int readEllipse(const char *equation, float *h, float *k, float *a, float *b) {
    // Parses ellipse equation of form ((x-h)^2/a^2) + ((y-k)^2/b^2) = 1
    // Returns parameters through pointer variables
...
int readParabola(const char *equation, float *a, float *b, float *c) {
    // Parses parabola equation of form y = ax^2 + bx + c
    // Returns parameters through pointer variables
...
int readHyperbola(const char *equation, float *h, float *k, float *a, float *b) {
    // Parses hyperbola equation of form ((x-h)^2/a^2) - ((y-k)^2/b^2) = 1
    // Returns parameters through pointer variables
    *h = 0.0f; *k = 0.0f; *a = 1.0f; *b = 1.0f;
    char cleanEq[MAX_INPUT_LENGTH];
    strcpy(cleanEq, equation);
...
void drawGrid(double minorSpacing, double majorSpacing) {
    // Draws coordinate grid with specified spacing
...
void plotFunction(const char* func, double r, double g, double b) {
    // Plots mathematical function with specified color
    // Handles domain restrictions and screen boundaries
    iSetColor(r, g, b);
    double startX = -offsetX - (WINDOW_WIDTH / 2) / scaleX;
    double endX = -offsetX + (WINDOW_WIDTH / 2) / scaleX;
...
void plotCircle(double r, double g, double b) {
    // Plots circle with specified color
    iSetColor(r, g, b);
...
void plotEllipse(double r, double g, double b) {
    // Plots ellipse with specified color
    iSetColor(r, g, b);
...
void plotParabola(double r, double g, double b) {
    // Plots parabola with specified color
    iSetColor(r, g, b);
    double startX = -offsetX - (WINDOW_WIDTH / 2) / scaleX;
    double endX = -offsetX + (WINDOW_WIDTH / 2) / scaleX;
...
void plotHyperbola(double r, double g, double b) {
    // Plots hyperbola with specified color
    iSetColor(r, g, b);
...
void drawUI() {
    ...
    if (showHelpScreen) {
        ...
        iText(60, WINDOW_HEIGHT-260, "[ ]: Zoom in/out | Arrows/Mouse: Pan | g: Toggle grid");
        iText(60, WINDOW_HEIGHT-300, "Press 'h' to close help");
    }

    // Show color setting UI only when active
    if (isSettingColor) {
        // Increase menu height to accommodate all items
        int menuWidth = WINDOW_WIDTH/2;
        int menuHeight = WINDOW_HEIGHT/2 + 50; // Increased height
        int menuX = WINDOW_WIDTH/4;
        int menuY = WINDOW_HEIGHT/4 - 25; // Adjusted Y position

        // Dark background with adjusted dimensions
        iSetColor(0, 0, 0);
        iFilledRectangle(menuX, menuY, menuWidth, menuHeight);
        // Border
        iSetColor(100, 100, 100);
        iRectangle(menuX, menuY, menuWidth, menuHeight);
        
        iSetColor(255, 255, 255);
        if (inputStep == 0) {
            int startY = WINDOW_HEIGHT/2 + 140; // Increased starting Y position
            int lineHeight = 25;
            int colWidth = 120;
            int startX = WINDOW_WIDTH/4 + 20;

            // Title with more space at top
            iText(startX, startY, "Select function to change color:");
            startY -= 2 * lineHeight;

            // Basic Trigonometric Functions
            iText(startX, startY, "Basic Trigonometric:");
            startY -= lineHeight;
            iSetColor(colorSin.r, colorSin.g, colorSin.b);
            iText(startX, startY, "1: Sin");
            iSetColor(colorCos.r, colorCos.g, colorCos.b);
            iText(startX + colWidth, startY, "2: Cos");
            iSetColor(colorTan.r, colorTan.g, colorTan.b);
            iText(startX + 2 * colWidth, startY, "3: Tan");
            startY -= 1.5 * lineHeight;

            // Inverse Trigonometric Functions
            iSetColor(255, 255, 255);
            iText(startX, startY, "Inverse Trigonometric:");
            startY -= lineHeight;
            iSetColor(colorASin.r, colorASin.g, colorASin.b);
            iText(startX, startY, "A: ASin");
            iSetColor(colorACos.r, colorACos.g, colorACos.b);
            iText(startX + colWidth, startY, "S: ACos");
            iSetColor(colorATan.r, colorATan.g, colorATan.b);
            iText(startX + 2 * colWidth, startY, "D: ATan");
            startY -= 1.5 * lineHeight;

            // Other Functions
            iSetColor(255, 255, 255);
            iText(startX, startY, "Other Functions:");
            startY -= lineHeight;
            iSetColor(colorExp.r, colorExp.g, colorExp.b);
            iText(startX, startY, "4: Exp");
            iSetColor(colorLog.r, colorLog.g, colorLog.b);
            iText(startX + colWidth, startY, "5: Log");
            iSetColor(colorLn.r, colorLn.g, colorLn.b);
            iText(startX + 2 * colWidth, startY, "6: Ln");
            startY -= 1.5 * lineHeight;

            // Shapes section with proper spacing
            iSetColor(255, 255, 255);
            iText(startX, startY, "Shapes:");
            startY -= lineHeight;
            
            // First row of shapes - maintain alignment
            iSetColor(colorPoly.r, colorPoly.g, colorPoly.b);
            iText(startX, startY, "7: Poly");
            iSetColor(colorCircle.r, colorCircle.g, colorCircle.b);
            iText(startX + colWidth, startY, "8: Circle");
            iSetColor(colorEllipse.r, colorEllipse.g, colorEllipse.b);
            iText(startX + 2 * colWidth, startY, "9: Ellipse");
            startY -= lineHeight;
            
            // Second row of shapes - maintain alignment with proper spacing
            iSetColor(colorParabola.r, colorParabola.g, colorParabola.b);
            iText(startX, startY, "0: Para");
            iSetColor(colorHyperbola.r, colorHyperbola.g, colorHyperbola.b);
            iText(startX + colWidth, startY, "-: Hyper");
            startY -= lineHeight;

            // ESC instruction with proper bottom margin
            iSetColor(150, 150, 150);
            iText(startX, menuY - 20, "Press ESC to cancel");
        } else {
            char prompt[50];
            ...
            if (strcmp(selectedFunction, "cos") == 0) targetColor = &colorCos;
            if (strcmp(selectedFunction, "tan") == 0) targetColor = &colorTan;
            if (strcmp(selectedFunction, "asin") == 0) targetColor = &colorASin;
            if (strcmp(selectedFunction, "acos") == 0) targetColor = &colorACos;
            if (strcmp(selectedFunction, "atan") == 0) targetColor = &colorATan;
            if (strcmp(selectedFunction, "exp") == 0) targetColor = &colorExp;
            if (strcmp(selectedFunction, "log") == 0) targetColor = &colorLog;
            if (strcmp(selectedFunction, "ln") == 0) targetColor = &colorLn;
            if (strcmp(selectedFunction, "poly") == 0) targetColor = &colorPoly;
            if (strcmp(selectedFunction, "circle") == 0) targetColor = &colorCircle;
            if (strcmp(selectedFunction, "ellipse") == 0) targetColor = &colorEllipse;
            if (strcmp(selectedFunction, "parabola") == 0) targetColor = &colorParabola;
            if (strcmp(selectedFunction, "hyperbola") == 0) targetColor = &colorHyperbola;
            
            if (targetColor) {
                iSetColor(255, 255, 255);
                char currentRGB[50];
                sprintf(currentRGB, "Current: R:%d G:%d B:%d", 
                        (int)targetColor->r, (int)targetColor->g, (int)targetColor->b);
                iText(WINDOW_WIDTH/4 + 20, WINDOW_HEIGHT/2 - 100, currentRGB);
                
                // Show color preview
                iSetColor(targetColor->r, targetColor->g, targetColor->b);
                iFilledRectangle(WINDOW_WIDTH/4 + 250, WINDOW_HEIGHT/2 - 80, 60, 60);
                iSetColor(200, 200, 200);
                iRectangle(WINDOW_WIDTH/4 + 250, WINDOW_HEIGHT/2 - 80, 60, 60);
            }
        }
    }
    ...
    if (isEnteringEquation) {
        ...
        iSetColor(255, 255, 255);
        // Draw pill-shaped input box with white background
        int boxHeight = 40;
        int boxWidth = WINDOW_WIDTH/3 + 120;
        int boxX = 207;
        int boxY = WINDOW_HEIGHT - 95;
        int radius = boxHeight/2;

        iSetColor(255, 255, 255);
        // Draw left circle
        iFilledCircle(boxX + radius, boxY + radius, radius);
        // Draw center rectangle
        iFilledRectangle(boxX + radius, boxY, boxWidth - 2*radius, boxHeight);
        // Draw right circle
        iFilledCircle(boxX + boxWidth - radius, boxY + radius, radius);
        ...
        if (format[0] != '\'\0') {
            ...
            iSetColor(150, 150, 150);
            iText(230, WINDOW_HEIGHT - 70, example);
        }
        

        iSetColor(0, 0, 0);
        iText(230, WINDOW_HEIGHT - 85, equationInput);
...
void clearAllGraphs() {
    // Clears all active function displays
    showSin = false;
    showCos = false;
    showTan = false;
    showASin = false;
...
void iKeyboard(unsigned char key) {
    // Handles keyboard input for function selection,
    // color changes, and view control
    ...
    if (isSettingColor) {
        if (inputStep == 0) {
            // Handle function selection with number keys
            switch(key) {
                // Basic Trig
                case '1': strcpy(selectedFunction, "sin"); inputStep++; break;
                case '2': strcpy(selectedFunction, "cos"); inputStep++; break;
                case '3': strcpy(selectedFunction, "tan"); inputStep++; break;
                
                // Inverse Trig (using A, S, D keys)
                case 'a': case 'A': strcpy(selectedFunction, "asin"); inputStep++; break;
                case 's': case 'S': strcpy(selectedFunction, "acos"); inputStep++; break;
                case 'd': case 'D': strcpy(selectedFunction, "atan"); inputStep++; break;
                
                // Other Functions
                case '4': strcpy(selectedFunction, "exp"); inputStep++; break;
                case '5': strcpy(selectedFunction, "log"); inputStep++; break;
                case '6': strcpy(selectedFunction, "ln"); inputStep++; break;
                
                // Shapes
                case '7': strcpy(selectedFunction, "poly"); inputStep++; break;
                case '8': strcpy(selectedFunction, "circle"); inputStep++; break;
                case '9': strcpy(selectedFunction, "ellipse"); inputStep++; break;
                case '0': strcpy(selectedFunction, "parabola"); inputStep++; break;
                case '-': strcpy(selectedFunction, "hyperbola"); inputStep++; break;
            ...
            inputBuffer[0] = '\0'; // Clear input buffer after selection
            return;
        }
        if (key == '\r') { // Enter key
        ...
        else if (key == '\b') { // Add backspace handling
            ...
            // Only allow numeric input for RGB values
            if (inputStep > 0 && !isdigit(key)) return;
...
void iMouse(int button, int state, int mx, int my) {
    // Handles mouse input for panning and UI interaction
...
void iSpecialKeyboard(unsigned char key) {
    // Handles special keyboard input for panning
    switch (key) {
    case GLUT_KEY_LEFT:
...
// Main Program Entry
int main() {
    // Initialize with Neon color preset
    applyColorPreset(currentPreset);
    // Create and show main window
    iInitialize(WINDOW_WIDTH, WINDOW_HEIGHT, "Graph Plotter by Arefin");
